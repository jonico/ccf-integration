...
public class TFSGenericParticipantConfigValidator implements IGenericParticipantConfigItemValidator {
    ...
    public void validate(AbstractGenericParticipantModel model, Errors errors) {
        ...
        validateValue(landscapeConfigList, errors,
                LANDSCAPE_CONFIG_LIST_ELEMENT_NAME);
        validateValue(participantConfigList, errors,
    ...
    public ValidationResult validateConnection(
        ...
        List<CCFCoreProperty> landscapeConfigList = model
                .getLandscapeConfigList();
        List<CCFCoreProperty> partcipantConfigList = model
                .getParticipantConfigList();
        ...
        if (StringUtils.isEmpty(userName) || StringUtils.isEmpty(url)) {
            ...
            try {
                configurationServer = new TFSTeamProjectCollection(url,
                        userName, domain, password);
                configurationServer.authenticate();
                return new ValidationResult(
                        configurationServer.hasAuthenticated());
            } catch (TFSUnauthorizedException e) {
                return new ValidationResult(false, e.getMessage());
            } catch (Exception e) {
                return new ValidationResult(false, e.getMessage());
            }/*
              * finally{ if(configurationServer != null)
              * configurationServer.close(); }
              */
        }
    }

    public void validateValue(List<CCFCoreProperty> configProperties,
        ...
        for (int i = 0; i < configProperties.size(); i++) {
            ...
            if (StringUtils.isEmpty(value)) {
                errors.rejectValue(errorElementName + "[" + i + "].value",
                        ...
                        "Blank value not accepted");
            } else if (!ValidatorUtils.validateType(value, type)) {
                errors.rejectValue(errorElementName + "[" + i + "].value", "",
                        "Value should be numeric"); // Numeric validation message will always be expected
            } else if (!ValidatorUtils.findMatch(conditionalRegex, value)) {
                errors.rejectValue(errorElementName + "[" + i + "].value", "",
                        "Value given is not matching conditional regex");
            }
        }
    }
