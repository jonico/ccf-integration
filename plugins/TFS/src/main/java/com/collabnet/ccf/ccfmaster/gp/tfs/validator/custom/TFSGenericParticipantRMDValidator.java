...
public class TFSGenericParticipantRMDValidator implements IGenericParticipantRMDValidator {
    ...
    public TFSGenericParticipantRMDValidator(String urlKey, String userNameKey,
            String passwordKey) {
        this.urlKey = urlKey;
    ...
    public void validate(RMDModel model, Errors errors) {
        ...
        ConfigItemValidatorUtils.validateValue(configProperties, errors,
                RMD_CONFIG_LIST_ELEMENT_NAME);
        for (int i = 0; i < configProperties.size(); i++) {
            ...
            if (name.equalsIgnoreCase("tfsCollection")) {
                ...
                boolean isvalid = tfsMetadataHelper.getTFSCollectionList()
                        .contains(collectionName);
                if (!isvalid) {
                    errors.rejectValue(RMD_CONFIG_LIST_ELEMENT_NAME + "[" + i
                            + "].value", "", "Collection is not found");
                }
                continue;
            }
            if (name.equalsIgnoreCase("tfsProjectList")) {
                boolean isvalid = tfsMetadataHelper.getTFSProjectList(
                        collectionName).contains(
                        configProperties.get(i).getValue());
                if (!isvalid) {
                    errors.rejectValue(RMD_CONFIG_LIST_ELEMENT_NAME + "[" + i
                            + "].value", "",
                            "Project for given collection is not found");
                }
                continue;
            }
            if (name.equalsIgnoreCase("tfsWorkItemType")) {
                boolean isvalid = Arrays.asList(
                        new String[] { "User Story", "Task", "Bug" }).contains(
                        configProperties.get(i).getValue());
                if (!isvalid) {
                    errors.rejectValue(RMD_CONFIG_LIST_ELEMENT_NAME + "[" + i
                            + "].value", "",
                            "WorkItem for given project is not found");
                }
                continue;
            }
        }
    ...
    private void buildTFSMetadatahelperAttr(RMDModel model) {
        ...
        tfsMetadataHelper.setPassword(model.getLandscapeConfigMap().get(
                passwordKey));
    }
